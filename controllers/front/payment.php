 ...
 * @property BankartPaymentGateway module
 */
class BankartPaymentGatewayPaymentModuleFrontController extends ModuleFrontController
{
    /**
     * @var Order
     */
    ...
    public function postProcess()
    {
        ...
        $this->module->validateOrder(
            $cart->id,
            \Configuration::get(BankartPaymentGateway::BANKART_PAYMENT_GATEWAY_OS_STARTING),
            $cart->getOrderTotal(true),
            $this->paymentTitles[$paymentType], // change to nice title
            null,
            [],
            null,
            false,
        ...
        if ($paymentResult->isSuccess()) {
            ...
            // logs  errors (credential failures etc.) that might not be visible via gateway's web logs)
            $errors = $paymentResult->getErrors();
            foreach($errors as $error) {
                if(in_array($error->getCode()[0], [1,3,9])) PrestaShopLogger::addLog('Bankart: payment result - ' . $error->getMessage(), 1, $error->getCode(), 'Order', (int) $this->order->id, true);
            }
    ...
    {
        if ($order->current_state == Configuration::get(BankartPaymentGateway::BANKART_PAYMENT_GATEWAY_OS_STARTING)) {
